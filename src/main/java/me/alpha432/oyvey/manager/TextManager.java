...
public class TextManager
    ...
    private CustomFont customFont = new CustomFont(new Font("Arial", 0, 17), true, false);
    ...
    public void init(boolean startup) {
        FontMod cFont = Sped.moduleManager.getModuleByClass(FontMod.class);
        try {
    ...
    public float drawString(String text, float x, float y, int color, boolean shadow) {
        if (Sped.moduleManager.isModuleEnabled(FontMod.getInstance().getName())) {
            if (shadow) {
