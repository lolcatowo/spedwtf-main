...
public class ConfigManager implements Util {
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        Sped.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Sped.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        Sped.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void init() {
        this.features.addAll( Sped.moduleManager.modules);
        this.features.add( Sped.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        Sped.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            Sped.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
