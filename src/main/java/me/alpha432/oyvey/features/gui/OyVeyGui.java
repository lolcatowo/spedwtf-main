...
public class OyVeyGui
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : Sped.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 94, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    Sped.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
