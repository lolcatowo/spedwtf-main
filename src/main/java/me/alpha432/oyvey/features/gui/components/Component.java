...
public class Component
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y - 1, this.x + this.width, this.y + this.height - 5.5f, ClickGui.getInstance().rainbow.getValue() != false ? ColorUtil.rainbow(ClickGui.getInstance().rainbowHue.getValue()).getRGB() : color);

        if (this.open) {
            RenderUtil.drawRect(this.x, (float) this.y + 12f, this.x + this.width, (float) (this.y + this.height) + totalItemHeight, 0x77000000);
        }
        Sped.textManager.drawStringWithShadow (this.getName() + ChatFormatting.AQUA , (float) this.x + 3.0f, (float) this.y - 4.0f - (float) OyVeyGui.getClickGui().getTextOffset(), -1);
        if (this.open) {
