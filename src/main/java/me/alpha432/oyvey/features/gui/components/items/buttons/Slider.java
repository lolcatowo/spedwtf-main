...
public class Slider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? Sped.colorManager.getColorWithAlpha(Sped.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : Sped.colorManager.getColorWithAlpha(Sped.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        Sped.textManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.AQUA + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf(((Number) this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) OyVeyGui.getClickGui().getTextOffset(), -1);
    }
